...
export default function SubtopicPage() {
  ...
  const quizPreview = problems.slice(0, 3);
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <motion.div
        ...
        className="mb-8 rounded-2xl border border-white/15 bg-card/60 p-5 backdrop-blur-xl"
      >
        ...
        <h1 className="text-3xl font-bold tracking-tight mb-3 sm:text-4xl">{subtopic.name}</h1>
        <p className="text-muted-foreground leading-relaxed mb-2">
          {subtopic.description}
        </p>
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <span className="inline-flex items-center gap-1 rounded-full border border-white/15 bg-background/60 px-2.5 py-1 text-muted-foreground">
            <Clock className="h-3 w-3" />
            {subtopic.estimatedMinutes} min
          </span>
          <span className="inline-flex items-center gap-1 rounded-full border border-athena-amber/25 bg-athena-amber/10 px-2.5 py-1 text-athena-amber">
            <Trophy className="h-3 w-3" />
            +120 XP mission
          </span>
          <span className="inline-flex items-center gap-1 rounded-full border border-emerald-400/25 bg-emerald-500/10 px-2.5 py-1 text-emerald-300">
      ...
      <motion.div
        ...
        className="mb-8 grid grid-cols-1 gap-3 lg:grid-cols-2"
      >
        <Link href={`/learning/${params.topicSlug}/${subtopic.slug}/micro-lesson`} className="block">
          <motion.div
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="group relative h-full border-2 border-athena-amber/40 bg-gradient-to-b from-athena-amber/10 to-transparent p-5 cursor-pointer transition-colors hover:border-athena-amber rounded-2xl"
          >
        ...
        {problems.length > 0 ? (
          <button onClick={() => router.push(`/learning/${params.topicSlug}/${params.subtopicSlug}/quiz`)} className="block text-left w-full">
            <motion.div
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="group relative h-full border-2 border-primary/40 bg-gradient-to-b from-primary/10 to-transparent p-5 cursor-pointer transition-colors hover:border-primary rounded-2xl"
            >
              ...
              <p className="text-xs text-muted-foreground mb-3">{problems.length} questions — SAT-style practice</p>
              <div className="mb-3 rounded-lg border border-white/10 bg-background/60 px-3 py-2 text-xs text-foreground/85">
                <p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Preview</p>
                <MathContent content={problems[0]?.questionText ?? ""} />
              </div>
              <div className="mb-4 flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-emerald-300">
                <Swords className="h-3 w-3" />
                Combo boosts and streak XP
              </div>
      ...
      {quizPreview.length > 0 && (
        <motion.div
          ...
          className="mb-8 rounded-2xl border border-cyan-400/20 bg-cyan-500/5 p-4"
        >
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs font-black uppercase tracking-widest text-cyan-300">Quiz Arena Preview</p>
            <button
              className="rounded-md border border-cyan-300/25 bg-cyan-500/10 px-2.5 py-1 text-[11px] font-bold text-cyan-200 hover:bg-cyan-500/20"
              onClick={() => router.push(`/learning/${params.topicSlug}/${params.subtopicSlug}/quiz/1`)}
            >
              Start at Q1
            </button>
          </div>
          <div className="space-y-2">
            {quizPreview.map((problem, index) => (
              <button
                key={problem.id}
                onClick={() => router.push(`/learning/${params.topicSlug}/${params.subtopicSlug}/quiz/${index + 1}`)}
                className="w-full rounded-xl border border-white/10 bg-background/65 px-3 py-2 text-left transition-colors hover:border-cyan-300/35"
              >
                <p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Question {index + 1}</p>
