          ...
          case "polygon": {
            ...
            return (
              <g key={`poly-${i}`}>
                <motion.polygon
                  points={pointsStr}
                  fill={fig.style?.fillColor ?? "var(--primary-foreground)"}
                  fillOpacity={0.1}
                  style={{ stroke: fig.style?.strokeColor ?? "var(--primary)" }}
                  strokeWidth={(fig.style?.strokeWidth ?? 2) * 1.5}
                  strokeLinejoin="round"
                  filter="url(#geo-glow) url(#sketchy)"
                ...
                {fig.vertexLabels?.map((label, j) => {
                  ...
                  return (
                    <text
                      ...
                      fontSize="14"
                      fontWeight="bold"
                      style={{ fill: fig.style?.strokeColor ?? "var(--primary)" }}
          ...
          case "circle": {
            ...
            return (
              <motion.circle
                ...
                style={{ stroke: fig.style?.strokeColor ?? "var(--athena-amber)" }}
                strokeWidth={(fig.style?.strokeWidth ?? 2) * 1.5}
                filter="url(#geo-glow) url(#sketchy)"
          ...
          case "angle_arc": {
            ...
            return (
              <g key={`arc-${i}`}>
                ...
                {ann.label && (
                  <text
                    x={vx + (r + 12) * Math.cos((fromAngle + toAngle) / 2)}
                    y={vy + (r + 12) * Math.sin((fromAngle + toAngle) / 2)}
                    textAnchor="middle"
                    dominantBaseline="middle"
                    fontSize="12"
                    fontWeight="bold"
