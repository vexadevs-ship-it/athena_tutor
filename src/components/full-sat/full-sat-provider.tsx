...
export function FullSatProvider({
  ...
  const finishSection = useCallback(() => {
    // Record R&W time
    const now = Date.now();
    const elapsed = Math.round((now - sectionStartRef.current) / 1000);
  ...
  }, [attempt.id, router, setRwTimeUsed]);
  ...
  const submitTest = useCallback(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [attempt.id, rwTimeUsed, mathTimeUsed, submitMutation, router, setMathTimeUsed]);
