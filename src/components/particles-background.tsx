...
export function ParticlesBackground() {
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      parallaxRef.current = { x: nx * 22, y: ny * 16 };
    };
    ...
    const animate = () => {
      ...
      const field = ctx.createRadialGradient(
        canvas.width * 0.3 + p.x,
        canvas.height * 0.2 + p.y,
        50,
        canvas.width * 0.3 + p.x,
        canvas.height * 0.2 + p.y,
        canvas.width * 0.8
      );
      field.addColorStop(0, isDark ? "rgba(14,165,233,0.10)" : "rgba(14,165,233,0.14)");
      field.addColorStop(1, "rgba(14,165,233,0)");
      ctx.fillStyle = field;
